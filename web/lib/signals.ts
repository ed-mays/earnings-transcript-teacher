...
/** Stream a news-context explanation for a single news item via SSE. */
export async function streamNewsContext(
  ticker: string,
  payload: { headline: string; summary: string; source: string; date: string },
  ...
  const response = await fetch(`${API_URL}/api/calls/${ticker}/news-context`, {
    method: "POST",
    headers,
